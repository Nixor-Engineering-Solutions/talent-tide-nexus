...
import { ChevronRight, Coins, Package, RefreshCw, Repeat } from "lucide-react";
...
export default function SPOnlyDetail({ listing, sellerProfile, reviews, gigId }: Props) {
  const { user } = useAuth();
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <div>
              <div className="flex items-center gap-2 mb-3 flex-wrap">
                <span className="flex items-center gap-1 text-xs font-mono text-badge-gold bg-badge-gold/10 px-2.5 py-1 rounded-lg">
                  <Coins className="w-3.5 h-3.5" />SP Only
                </span>
                ...
                {listing.is_subscription && <span className="flex items-center gap-1 text-xs font-mono text-teal-400 bg-teal-400/10 px-2 py-1 rounded-lg"><RefreshCw className="w-3 h-3" />Subscription</span>}
            ...
            {/* Price Display */}
            {!hasTiers && (
              <div className="rounded-2xl border border-badge-gold/20 bg-badge-gold/5 p-6 text-center">
                <p className="text-4xl font-mono font-black text-skill-green">{listing.points} SP</p>
                <p className="text-sm text-muted-foreground mt-2">One-time purchase</p>
                {listing.is_subscription && listing.subscription_interval && (
                  <p className="text-xs text-teal-400 mt-1">Billed {listing.subscription_interval}</p>
                )}
              </div>
            )}
            ...
            {hasTiers && (
              <div>
                <h3 className="text-xs font-mono uppercase tracking-widest text-muted-foreground mb-3">Choose Your Package</h3>
            ...
            <div className="sticky top-24 space-y-6">
              ...
              {listing.points > 0 && (
                <div className="rounded-xl bg-skill-green/5 border border-skill-green/20 p-4 text-center">
                  <p className="text-2xl font-mono font-bold text-skill-green">{hasTiers ? listing.tiers[selectedTier]?.price_sp : listing.points} SP</p>
                  <p className="text-xs text-muted-foreground mt-1">{hasTiers ? listing.tiers[selectedTier]?.name : "Price"}</p>
                </div>
              )}
              <DetailInteractionBar counts={counts} userState={userState} toggle={toggle} share={share} report={report} onMessage={() => {}} onPropose={handlePropose} ctaLabel="Purchase with SP" ctaIcon={<Coins className="w-4 h-4" />} ctaClassName="w-full h-12 rounded-xl bg-badge-gold text-background font-heading font-bold text-sm hover:bg-badge-gold/90 transition-colors flex items-center justify-center gap-2" />
            </div>
          </div>
        </div>
      </motion.div>
